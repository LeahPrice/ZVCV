...
aSECF <- function(integrands,samples,derivatives, polyorder = NULL, sigma = NULL, steinOrder = NULL, kernel_function = NULL,subset = NULL, K0 = NULL,nystrom_inds = NULL, conjugate_gradient = TRUE, reltol = 1e-02, est_inds = NULL, diagnostics = FALSE){
...
CF_crossval <- function(integrands, samples, derivatives, steinOrder = NULL, kernel_function = NULL, sigma_list = NULL, K0_list = NULL, folds = NULL, est_inds = NULL, one_in_denom = FALSE, diagnostics = FALSE){
...
SECF <- function(integrands,samples,derivatives, polyorder = NULL, steinOrder = NULL, kernel_function = NULL, sigma = NULL, K0 = NULL,subset = NULL, est_inds = NULL, diagnostics = FALSE){
...
SECF_crossval <- function(integrands,samples,derivatives, polyorder = NULL, steinOrder = NULL, kernel_function = NULL, sigma_list = NULL, K0_list = NULL, subset = NULL, folds = NULL, est_inds = NULL, diagnostics = FALSE){
...
aSECF_crossval <- function(integrands,samples,derivatives, polyorder = NULL, steinOrder = NULL, kernel_function = NULL, sigma_list = NULL, subset = NULL, folds = NULL, conjugate_gradient = TRUE, reltol = 1e-02, est_inds = NULL, diagnostics = FALSE){
  ...
  mse <- aSECF_crossval_cpp(integrands, samples, derivatives, getX = getX, aSECF_mse_linsolve = aSECF_mse_linsolve, polyorder, steinOrder, kernel_function, sigma_list, subset, folds, conjugate_gradient, reltol = reltol, est_inds = est_inds)
  ...
  for (j in unique(opt_indices)){
    ...
    temp <- aSECF(matrix(integrands[,inds],ncol=length(inds)), samples, derivatives, polyorder, sigma_list[[opt_indices[j]]], steinOrder, kernel_function, subset, NULL, NULL, conjugate_gradient, reltol, est_inds, diagnostics)
    
    if(diagnostics & (j==(unique(opt_indices))[1])){
      b <- matrix(NaN,nrow=length(temp$b),ncol=N_expectations)
      ny_inds <- matrix(NaN,nrow=length(temp$ny_inds),ncol=N_expectations)
    }
    ...
    if (diagnostics){
      ...
      b[,inds] <- temp$b
      if (length(inds)==1){
        ny_inds[,inds] <- temp$ny_inds
      } else{
        for (zz in 1:length(inds)){
          ny_inds[,inds[zz]] <- temp$ny_inds
        }
      }
    }
  }
